    ...
    link: function(scope, element, attrs, $window){
      ...
      function populateWindows(windowsStack){
        ...
        for(var i=0; i < windowsStack.length; ++i){
          addWindow(windowsStack[i].top, windowsStack[i].left, (windowsStack[i].height/GRID_SIZE).toFixed(2), (windowsStack[i].width/GRID_SIZE).toFixed(2), windowsStack[i].relativeHeight, windowsStack[i].relativeWidth);
        }
        constructWall();
