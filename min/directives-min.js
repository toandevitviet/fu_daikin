...
directives.directive("houseOverview", function(){
  return {
    restrict: "A",
    scope: false,
    link: function(scope, element, attrs, $window){
    	...
    	function calcHeight(){
            if($(".top-level").children().length > MAX_ROOM_PER_ROW){
                $("body").attr("data-more-rooms", "top");
            } else if ($(".bottom-level").children().length  > MAX_ROOM_PER_ROW){
        ...
        function animate(){
        	...
        	for(var i=0; i<roomsControls.length; ++i){
        		var control = roomsControls[i];
    	...
    	var topLevel = $('.top-level', element[0]);
    	var groundLevel = $('.ground-level', element[0]);
        ...
        var oneThirdHeight = $(element[0]).height()/3;

    	for(var i=0; i<rooms.length; ++i){
    		
            var roomView = $("<div class='a-room-view' data-id='"+i+"'><div class='a-room-header'><div class='a-room-type'>"+rooms[i].roomType+"</div><div class='a-room-handle'><img src='/img/handle.png'></div><a class='a-room-info' href='javascript:void(0)' data-room='" + i +"'>i</a></div></div>");
    		
            roomView.css("height", oneThirdHeight)

            if(rooms[i].topLevel){
                roomView.appendTo(topLevel);
                countTop++;
            } else {
                roomView.appendTo(groundLevel);
                countGround++;
            }
            ...
            roomsControls.push(init3DRoom(rooms[i], canvasContainer,  function(idx, currentLine){
          		return gray;
      		}, 2, true, renderer));

            roomView.draggable({handle: ".a-room-handle"});
    	}
        ...
        $('.top-level, .ground-level').droppable({
          ...
          drop: function( event, ui ) {
            ...
            room.appendTo(event.target).removeAttr("style").css("height", oneThirdHeight);
            calcHeight();
  ...
  return {
    ...
    link: function(scope, element, attrs, $window){
      ...
      function addWindow(top, left, height, width, relativeHeight, relativeWidth){
        ...
        var wallWindow = $("<div class='wall-window' data-id='"+windows.length+"'><div class='window-width-wrapper'><span class='window-width'></span> m</div><div class='window-height-wrapper'><span class='window-height'></span> m</div></div></div>");
      ...
      function undoFunction(){
        ...
        var undoWindows = undoStack.pop();
        for(var i=0; i < undoWindows.length; ++i){
          addWindow(undoWindows[i].top, undoWindows[i].left, (undoWindows[i].height/GRID_SIZE).toFixed(2), (undoWindows[i].width/GRID_SIZE).toFixed(2), undoWindows[i].relativeHeight, undoWindows[i].relativeWidth);
        }
      ...
      function getWidth(width, containerWidth, actualWidth){
        return ((width/_2DscaleFactor) / GRID_SIZE).toFixed(2);//(width/containerWidth * actualWidth).toFixed(2);
      }

      function getHeight(height, containerHeight, actualHeight){
        return ((height/_2DscaleFactor)/ GRID_SIZE).toFixed(2);
      }
      ...
      function constructWall(){
        ...
        for(var i=0; i < windows.length; ++i){
          ...
          var area = (getWidth(width) * getHeight(height)).toFixed(2);
      ...
      function onMove( e ){
        ...
        var eventX = position.left;
        if(eventX < 0)
          eventX = 0;
        else if(eventX + itemWidth > innerWallWidth){
          eventX = innerWallWidth - itemWidth;
        }
 
        var eventY = position.top;
        if(eventY < 0)
          eventY = 0;
        else if(eventY + itemHeight > innerWallHeight){
          eventY = innerWallHeight - itemHeight;
        }
      ...
      $(mainWall).droppable({
        ...
        drop: function( e, ui ) {
          ...
          var tempTop = eventY;
          var tempLeft = eventX;
          var tempHeight = GRID_SIZE * 6;
          var tempWidth = GRID_SIZE * 6;
      ...
      if(currLine.windows != null && currLine.windows.length > 0){
        ...
        for(var i=0; i < currLine.windows.length; ++i){
          addWindow(currLine.windows[i].top, currLine.windows[i].left, (currLine.windows[i].height/GRID_SIZE).toFixed(2), (currLine.windows[i].width/GRID_SIZE).toFixed(2), currLine.windows[i].relativeHeight, currLine.windows[i].relativeWidth);
        }
